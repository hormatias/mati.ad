...
test('Social preview has absolute metadata and a valid 1200×1200 PNG', () => {
  const html = readFileSync('index.html', 'utf8');
  const tags = Object.fromEntries([...html.matchAll(/<meta (?:property|name)="([^"]+)" content="([^"]*)"/g)].map(m => [m[1], m[2]]));
  assert.equal(tags['og:image'], 'https://mati.ad/images/og-mati-square-v2.png');
  ...
  const png = readFileSync('assets/images/og-mati-square-v2.png');
  assert.equal(png.subarray(0, 8).toString('hex'), '89504e470d0a1a0a');
  assert.equal(png.readUInt32BE(16), 1200);
  assert.equal(png.readUInt32BE(20), 1200);
  assert.equal(tags['og:image:width'], '1200');
  assert.equal(tags['og:image:height'], '1200');
